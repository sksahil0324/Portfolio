      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {certifications.map((cert, idx) => {
            ...
            return (
              <motion.div key={cert.id} initial={{ opacity: 0, scale: 0.92 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: idx * 0.08 }} whileHover={{ y: -4 }} className={`glass-card rounded-xl p-6 border transition-all duration-300 ${c.border}`} data-testid={`cert-${idx}`}>
