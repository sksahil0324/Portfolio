      ...
      <div className="container mx-auto px-4 max-w-6xl relative">
        ...
        <div className="space-y-10">
          {skills.map((category, catIdx) => (
            <motion.div key={category.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: catIdx * 0.1 }}>
              <h3 className={`text-sm font-mono uppercase tracking-widest mb-4 ${glowMap[category.color] ?? "text-primary"}`}>{category.label}</h3>
              <div className="flex flex-wrap gap-3">
                {category.skills.map((skill, skillIdx) => (
                  <motion.span
                    key={skill}
                    initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }}
                    viewport={{ once: true }} transition={{ duration: 0.3, delay: catIdx * 0.08 + skillIdx * 0.04 }}
